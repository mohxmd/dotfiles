return {
  ...
  init = function()
    local group = vim.api.nvim_create_augroup("ConfigObsidianMarkdown", { clear = true })
  ...
  config = function(_, opts)
    if #opts.workspaces == 0 then
      -- Obsidian.nvim requires a real vault. It is still available on
      -- demand once a configured vault is present.
      return
    end
